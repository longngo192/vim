vim.g.mapleader = ' '
local map = vim.api.nvim_set_keymap
map('n', '<C-h>', '<C-w>h', { noremap = true, silent = false })
map('n', '<C-l>', '<C-w>l', { noremap = true, silent = false })
map('n', '<C-j>', '<C-w>j', { noremap = true, silent = false })
map('n', '<C-k>', '<C-w>k', { noremap = true, silent = false })

map('i', '<C-h>', '<Left>', { noremap = true, silent = false })
map('i', '<C-l>', '<Right>', { noremap = true, silent = false })
map('i', '<C-j>', '<Down>', { noremap = true, silent = false })
map('i', '<C-k>', '<Up>', { noremap = true, silent = false })
map('n', '<C-w>', '<ESC>:bd<CR>', { noremap = true, silent = false })
map('i', '<C-w>', '<ESC>:w<CR>:bd<CR>', { noremap = true, silent = false })
map('n', 'dl', ':t.<CR>', { noremap = true, silent = false })
map('i', '<A-Down>', '<ESC>:m.+1<CR>i', { noremap = true, silent = false })
map('n', '<A-Down>', '<ESC>:m.+1<CR>i', { noremap = true, silent = false })
map('n', 'te', '<ESC>:tabedit', { noremap = true, silent = false })
map('i', '<A-Up>', '<ESC>:m.-2<CR>i', { noremap = true, silent = false })
map('n', '<A-Up>', '<ESC>:m.-2<CR>i', { noremap = true, silent = false })
map('i', 'kk', '<ESC>', { noremap = true, silent = false })
map('i', '<S-Right>', '<ESC>v<Right>', { noremap = true, silent = false })
map('n', '<S-Right>', '<ESC>v<Right>', { noremap = true, silent = false })
map('i', '<S-Left>', '<ESC>v<Left>', { noremap = true, silent = false })
map('n', '<S-Left>', '<ESC>v<Left>', { noremap = true, silent = false })
map('n', '<S-Up>', '<ESC>v<Up>', { noremap = true, silent = false })
map('n', '<S-Down>', '<ESC>v<Down>', { noremap = true, silent = false })
map('n', ',', '<ESC>0', { noremap = true, silent = false })
map('n', '.', '<ESC>$', { noremap = true, silent = false })
map('n', '<leader>h', ':Prettier<CR>', { noremap = true, silent = false })
map('i', '<leader>h', '<ESC>:Prettier<CR>i', { noremap = true, silent = false })
map('n', '<C-s>', '<ESC>:w<CR>', { noremap = true, silent = false })
map('i', '<C-s>', '<ESC>:w<CR>', { noremap = true, silent = false })
map('n', 'T', ':lua vim.lsp.buf.signature_help()<CR>', { noremap = true, silent = false })
map('n', '*', '<cmd>Lspsaga show_line_diagnostics<cr>', { noremap = true, silent = false })
map('n', 'gh', '<cmd>Lspsaga lsp_finder<CR>', { noremap = true, silent = false })
map('n', 'K', '<cmd>Lspsaga hover_doc<cr>', { noremap = true, silent = false })
map('n', '<leader>tr', ':source $MYVIMRC<cr>', { noremap = true, silent = false })
map('n', '<leader>gg', ':LazyGit<CR>', { noremap = true, silent = false })

map('n', '<leader>et', ':NvimTreeToggle<CR>', { noremap = true, silent = true })
map('n', '<leader>ef', ':NvimTreeFindFile<CR>', { noremap = true, silent = true })

map("n", "<C-Up>", "<cmd>lua require('lspsaga.action').smart_scroll_with_saga(-1)<cr>",
  { noremap = true, silent = true })
map("n", "<C-Down>", "<cmd>lua require('lspsaga.action').smart_scroll_with_saga(1)<cr>",
  { noremap = true, silent = true })
